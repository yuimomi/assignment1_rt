...
int main(int argc, char **argv){
    ros::init(argc, argv, "distance_code");
    ros::NodeHandle nh;
    ...
    pub_turtle1 = nh.advertise<std_msgs::Twist>("turtle1/cmd_vel",10);
    pub_turtle2 = nh.advertise<std_msgs::Twist>("turtle2/cmd_vel",10);
    ...
    while(ros::ok()){
        ...
        if (x1 < 1.0 || x1 > 10.0 || y1 < 1.0 || y1 > 10.0 || 
            x2 < 1.0 || x2 > 10.0 || y2 < 1.0 || y2 > 10.0) {
            geometry_msgs::Twist stop_cmd;
            stop_cmd.linear.x = 0;
            stop_cmd.angular.z = 0;
            pub_turtle1.publish(stop_cmd);
